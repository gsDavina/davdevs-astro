...
- [x] Run Lighthouse against the new Astro build — ran `lighthouse` CLI (headless Chrome) against `astro build`'s static output (served locally, since the Vercel-adapter output isn't `astro preview`-able directly) for home, one article detail, one ebook detail, and one tool page. Scores (min across all four, see `_internal-docs/lighthouse/*.report.{html,json}`): **performance 68, accessibility 95, best-practices 100, seo 92**. Found and fixed 3 real accessibility bugs in the process: `EmojiFoodCatcher`'s difficulty `<select>` had a visible-but-unassociated `<label>` (added `htmlFor`/`id`), the cookie-banner's "Learn more" link relied on color alone to read as a link (added `text-decoration: underline`), and the mobile tab-bar's inactive label color failed WCAG AA contrast in both themes (`--tab-label` darkened/lightened from ~1.7-1.9:1 to ~4.6-4.7:1 in `tokens.css`). **"Confirm scores meet or beat the original" is not verifiable** — davinaleong.com isn't reachable from this environment for a comparison run, so there's no baseline to beat; this audits our own build in isolation. A **separate, still-open finding**: the tool page's `color-contrast` audit still fails on several detail-page-furniture elements shared by every content page (kicker line, `tag-coral` pills, share-row links, footer links/logo/copyright) — a broader design-token contrast pass across `--text-faint`/`--footer-text`/the hashed tag-color palette, not yet done; tracked in `02-test-checklists.md` §12 (Accessibility) rather than silently left out
