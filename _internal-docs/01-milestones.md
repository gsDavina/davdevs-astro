...
- [x] Rebuild the top nav (`⌘K` trigger) and footer (wordmark, `perf/a11y/seo` badge, copyright, Privacy link, cookie banner) as shared layout components — originally matched the real nav exactly at **10 links** (Sermon/Template linking to real, empty listing pages — see design-reference correction note in `03-progress.md`); **updated 2026-09-07**: Sermon removed entirely per instruction (no nav link, no route — `/sermon` 404s), now **9 links**: Article, eBooks, Frontend Mentor, Knowledge Sharing, Notebooks, Project, Template, Tool, Funny
...
- [x] Home page: hero + "latest N + view all" sections, matching card styling per content type — E-Books pinned first, then one section per blog type in `HOME_SECTION_ORDER` (capped at 5 entries each, updated 2026-09-07 from an earlier, unenforced "latest 4" that had actually regressed to showing every entry — see `03-progress.md`); Template's section never renders (zero entries), matching the source's own `@if($entries->isNotEmpty())` behaviour; Sermon isn't in `HOME_SECTION_ORDER` at all since its nav link/route were removed
...
- [x] Confirm search covers all 10 content types plus pages, matching original result quality — covers all 7 types that have actual content (6 blog types + E-Books); Template has zero entries to index (not a gap), Sermon has no route to index at all (removed entirely 2026-09-07), quips are intentionally excluded (no single canonical URL per quip); "matching original result quality" isn't independently verifiable without live-site access
...
- [x] Reproduce the footer `perf · a11y · seo` score display — confirm data source (live Lighthouse run, stored report, or build-time check) and replicate — badge UI built (`src/lib/site-config.ts` + `Footer.astro`), now showing **real** scores from a local audit (see next item), no longer placeholders
- [x] Run Lighthouse against the new Astro build — ran `lighthouse` CLI (headless Chrome) against `astro build`'s static output (served locally, since the Vercel-adapter output isn't `astro preview`-able directly) for home, one article detail, one ebook detail, and one tool page. Final scores (min across all four, see `_internal-docs/lighthouse/*.report.{html,json}`): **performance 70, accessibility 100, best-practices 100, seo 92**. Two rounds of real fixes verified by re-running the audit after each: (1) `EmojiFoodCatcher`'s difficulty `<select>` had a visible-but-unassociated `<label>` (added `htmlFor`/`id`), the cookie-banner's "Learn more" link relied on color alone (added `text-decoration: underline`), and the mobile tab-bar's inactive label color failed WCAG AA contrast in both themes; (2) a follow-up pass on the *same* `color-contrast` audit found it still failing on shared detail-page furniture (kicker line via `--text-faint`, share-row links via `--text-muted`, footer links/logo/copyright via `--footer-text`, and `tag-coral` pills via `--tertiary`, which — unlike `--accent`/`--secondary` — had never been given a separate, darker light-theme value). All four tokens retuned in `tokens.css` (each contrast ratio hand-computed before touching the token, then confirmed by re-running Lighthouse): accessibility went from 84 → 95 → **100**, zero remaining `color-contrast` failures. **"Confirm scores meet or beat the original" is not verifiable** — davinaleong.com isn't reachable from this environment for a comparison run, so there's no baseline to beat; this audits our own build in isolation
