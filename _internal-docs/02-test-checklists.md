...
- [x] Top nav (all links + `⌘K`) matches spacing, order, and mobile collapse behavior — originally 10 links including Sermon/Template; **updated 2026-09-07**: both removed entirely per instruction (`/sermon` and `/template` both 404 now), verified in-browser the nav renders exactly 8 links (Article, eBooks, Frontend Mentor, Knowledge Sharing, Notebooks, Project, Tool, Funny); `⌘K` and mobile collapse (nav → logo + tab bar) both verified in-browser
...
- [x] All top-nav links resolve to the correct listing pages — all 8 links (see §1), all verified to route correctly; neither Sermon nor Template have a link or route anymore (both removed 2026-09-07)
...
- [x] Template — **N/A**, zero live entries existed to migrate (see 01-milestones.md Phase 0); as of 2026-09-07 there's also no nav link or route at all (`/template` 404s, removed per instruction rather than kept as an empty placeholder)
...
- [x] Search covers all 10 content types and returns relevant results — covers all 7 types that have actual content (6 blog types + E-Books); Sermon and Template have no routes to index at all (both removed 2026-09-07), quips are intentionally excluded (no single canonical URL per quip, see Phase 7 note)
